...
// return normal in world-space for the given pixel 为给定的像素返回世界空间中的法线。
// the 3D points can be precomputed and passed here 可以预先计算并传递3D点。
void DepthData::GetNormal(const ImageRef& ir, Point3f& N, const TImage<Point3f>* pPointMap) const
{
	...
	if (!normalMap.empty()) {
		// set available normal 设置可用的法线。
		N = camera.R.t()*Cast<REAL>(normalMap(ir));
		return;
	}
	// estimate normal based on the neighbor depths 基于邻居深度估计法线。
...
float DepthEstimator::ScorePixelImage(const ViewData& image1, Depth depth, const Normal& normal)
{
	...
	ASSERT(n == nTexels);
	// score similarity of the reference and target texture patches 计算参考纹理补丁和目标纹理补丁之间的相似度得分。
	#if DENSE_NCC == DENSE_NCC_FAST
...
// the solution belonging to the target image can be also propagated 目标图像的解也可以进行传播。
void DepthEstimator::ProcessPixel(IDX idx)
{
	// compute pixel coordinates from pixel index and its neighbors 根据像素索引和其邻居计算像素坐标。
	...
	#endif
	if (dir == LT2RB) {
		// direction from left-top to right-bottom corner 从左上角到右下角的方向。
	...
	// check if any of the neighbor estimates are better then the current estimate 检查邻居估计是否有比当前估计更好的估计。
	#if DENSE_SMOOTHNESS != DENSE_SMOOTHNESS_NA
	FOREACH(n, neighbors) {
	...
	for (NeighborData& neighbor: neighbors) {
		...
		#endif
		// compute pixel's NCC score
		//该函数计算一个给定深度和法线下的像素的NCC分数，以评估深度估计的质量。
	...
	// try random values around the current estimate in order to refine it 尝试在当前估计周围随机取值，以便对其进行精细调整。
	...
	if (conf <= thConfSmall)
		...
		// try completely random values in order to find an initial estimate 尝试完全随机的值，以找到一个初始估计。
	...
	#else
	// current pixel estimate 当前像素的估计。
	PixelEstimate currEstimate{depth, normal};
	// propagate depth estimate from the best neighbor estimate 从最佳邻居估计中传播深度估计。
...
// triangulate in-view points, generating a 2D mesh 对视图内的点进行三角测量，生成一个二维网格
// return also the estimated depth boundaries (min and max depth) 同时返回估计的深度边界（最小深度和最大深度）
std::pair<float,float> TriangulatePointsDelaunay(const DepthData::ViewData& image, const PointCloud& pointcloud, const IndexArr& points, CGAL::Delaunay& delaunay)
{
	...
	// if full size depth-map requested 如果请求完整尺寸的深度图
	if (OPTDENSE::bAddCorners) {
		...
		// add the four image corners at the average depth 将四个图像角点添加到平均深度处
		...
		for (int i=0; i<4; ++i) {
			...
			if (cfc == 0)
				continue; // normally this should never happen 通常情况下，这种情况不应该发生
...
// roughly estimate depth and normal maps by triangulating the sparse point cloud 通过对稀疏点云进行三角测量，粗略估计深度和法线图
	...
	DepthMap& depthMap, NormalMap& normalMap, Depth& dMin, Depth& dMax)
{
	...
	// triangulate in-view points 对视图内的点进行三角测量
	CGAL::Delaunay delaunay;
	// 首先使用 Delaunay 算法对可见的 3D 点进行三角剖分，然后返回所得三角形的最小和最大深度。
	const std::pair<float,float> thDepth(TriangulatePointsDelaunay(image, pointcloud, points, delaunay));
	dMin = thDepth.first;
	dMax = thDepth.second;

    //然后，它通过在每个三角形内插值深度和法线值，创建一个粗略的深度图和法线图。
	// create rough depth-map by interpolating inside triangles 通过在三角形内进行插值创建粗略的深度图
	...
	struct RasterDepthDataPlaneData {
		...
		inline void operator()(const ImageRef& pt) {
			...
			if (z <= 0) // due to numerical instability 由于数值不稳定性的原因
				return;
			depthMap(pt) = z;
			normalMap(pt) = normal;
		}
	...
	for (CGAL::Delaunay::Face_iterator it=delaunay.faces_begin(); it!=delaunay.faces_end(); ++it) {
		...
		// compute the plane defined by the 3 points 计算由这3个点定义的平面
		// 计算有三个点定义的平面
		...
		// 计算edge1和edge2的叉积，即该三角形的法向量，并对其进行归一化
		data.normal = normalized(edge2.cross(edge1));
		// 计算该三角形所在平面的法向量，并将其乘上与该平面距离的倒数，将结果赋值给data.normalPlane
		data.normalPlane = data.normal * INVERT(data.normal.dot(c0));
		// draw triangle and for each pixel compute depth as the ray intersection with the plane 绘制三角形，并对每个像素计算深度，作为射线与平面的交点
...
// same as above, but does not estimate the normal-map 与上述相同，但不估计法线图
bool MVS::TriangulatePoints2DepthMap(
	const DepthData::ViewData& image, const PointCloud& pointcloud, const IndexArr& points,
