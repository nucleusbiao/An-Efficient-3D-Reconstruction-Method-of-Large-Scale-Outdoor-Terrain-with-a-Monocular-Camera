...
// compute visibility for the reference image 计算参考图像的可见性
// and select the best views for reconstructing the dense point-cloud; 并选择最适合重建密集点云的最佳视图
// extract also all 3D points seen by the reference image; 同时提取参考图像所看到的所有3D点
...
bool Scene::SelectNeighborViews(uint32_t ID, IndexArr& points, unsigned nMinViews, unsigned nMinPointViews, float fOptimAngle)
{
	...
	// extract the estimated 3D points and the corresponding 2D projections for the reference image 提取参考图像的估计3D点和相应的2D投影
	Image& imageData = images[ID];
	...
	CLISTDEF0(Score) scores(images.GetSize());//有多少图像就有多少个分数
	scores.Memset(0);
	...
	FOREACH(idx, pointcloud.points) {
		...
		++nPoints;
		// score shared views 评分共享视图
		const Point3f V1(imageData.camera.C - Cast<REAL>(point)); // 得到参考图像从相机中心到该3D点的向量
		...
		for (const PointCloud::View& view: views) {
			...
			const Point3f V2(imageData2.camera.C - Cast<REAL>(point)); // 得到视图2从相机中心到该3D点的向量
			const float fAngle(ACOS(ComputeAngle<float,float>(V1.ptr(), V2.ptr())));//计算V1视角与视角2的角度差；
			const float wAngle(MINF(POW(fAngle/fOptimAngle, 1.5f), 1.f));// 计算角度权重，通过将角度差除以一个优化参数fOptimAngle=10，并取幂次1.5后做一个最小函数（即权重最小为1），来减弱大角度差的影响；
			const float footprint2(Footprint(imageData2.camera, point));//计算点 point 在图像2的焦距/深度
			const float fScaleRatio(footprint1/footprint2);//计算当前点在视角1与视角2的尺度之比；
			float wScale;//计算重叠度权重。
			//fScaleRatio表示两个视角在该点上的投影面积比例。wScale是根据该比例计算的权重
			if (fScaleRatio > 1.6f)
			...
			// 对每个视图，根据每个共享3D点得到与参考图像的分数，3D点分数的累加即为该视图的评分。
	...
	// select best neighborViews 选择最佳的邻居视图
	...
	FOREACH(IDB, images) {
		...
		if (score.points < 3)
			continue;
		ASSERT(ID != IDB);
		// compute how well the matched features are spread out (image covered area) 计算匹配特征的分布情况（图像覆盖区域）的好坏程度
		...
		if (projs.IsEmpty())
			continue;
		//使用 ComputeCoveredArea 函数计算 projs 数组中的点在 imageData 中对应的区域的面积 area
		const float area(ComputeCoveredArea<float,2,16,false>((const float*)projs.Begin(), projs.GetSize(), boundsA.ptr()));
		projs.Empty();
		// store image score 存储图像得分
		//将每个匹配图像的得分和相关信息存储在neighbors数组中
...
// keep only the best neighbors for the reference image 仅保留参考图像的最佳邻居
