...
bool SerializeLoad(_Tp& obj, const std::string& fileName, uint32_t* pVersion=NULL) {
	...
	if (!stream)
		return false;
	// std::cout << szHeader << std::endl; MVSI
	// std::cout << "MVSI_PROJECT_ID = " << MVSI_PROJECT_ID << std::endl; MVSI
	if (strncmp(szHeader, MVSI_PROJECT_ID, 4) != 0) {
		...
		// std::cout << "version: " << version << std::endl; 2
		if (!stream || version > MVSI_PROJECT_VER)
			return false;
		// skip reserved bytes
		// std::cout << "MVSI_PROJECT_VER: " << MVSI_PROJECT_VER << std::endl; 5
